...
write_measurementchains_data <- function(mc_data,
                                         target_directory, 
                                         to_zip = FALSE,
                                         debug = FALSE) {
...
mc_data_stats <- mc_data %>%  
  dplyr::summarise(datetime_min = min(.data$datum_uhrzeit), 
...
csv_path <- sprintf("%s/mc-data_%s_%s.csv",
...
msg <- sprintf("Exporting provided dataset '%s' to '%s'",
               dataset_name, 
               csv_path)
...
if(to_zip) {
  zip_path <- stringr::str_replace(csv_path, 
                                  "\\.csv", 
                                  ".zip")
  
  msg <- sprintf("Exporting provided dataset '%s' to '%s' and delete intermediate '%s'",
                 dataset_name, 
                 zip_path, 
                 csv_path)
