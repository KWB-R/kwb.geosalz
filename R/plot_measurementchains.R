...
plot_measurementchains <- function(mc_data, para = "Leitfaehigkeit") {
  ...
  stats::setNames(lapply(seq_len(length(well_ids)), function(i) {
  dat %>% 
  dplyr::filter(.data$brunnen_nummer == well_ids[i]) %>% 
  dplyr::filter(.data$datum_uhrzeit >= as.POSIXct("2022-09-27 11:00:00", 
                                                 tz = "Etc/GMT-1")) %>% 
  dplyr::mutate(label = as.factor(sprintf("%s m uGOK (%s)", 
                                .data$einbau_sensor_muGOK, 
                                .data$sensor_id)) %>%  
                  forcats::fct_reorder(.data$einbau_sensor_muGOK)) %>% 
  dplyr::filter(.data$parameter == para) %>%  
  ggplot2::ggplot(mapping = ggplot2::aes(x = .data$datum_uhrzeit, 
                                         y = .data$messwert, 
                                         col = .data$label)) +
