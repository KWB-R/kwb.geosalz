if (FALSE)
{
  ...
  shp_svm_path <- system.file(
    "extdata/gis/shapefiles/svm.shp", 
    package = "kwb.geosalz"
  )
  ...
  shp_fri <- shapefiles::read.shapefile(
    stringr::str_remove(shp_svm_path, "\\.shp")
  )
  ...
  #   kwb.utils::catAndRun(
  #     messageText = "Removing duplicated data points",
  #     expr = {
  #       shp_fri$shp$shp[[1]]$points <- shp_fri$shp$shp[[1]]$points[!is_duplicated,]
  #     }
  #   )
  #   kwb.utils::catAndRun(
  #     messageText = "Writting cleaned shapefile",
  #     expr = {
  ...
  )
  {
    df %>% 
      dplyr::mutate(
        label = wasserportal::columns_to_labels(
          data = df, 
          columns = columns,
          fmt = "<p>%s: %s</p>",
          sep = ""
  ...
  readr::write_csv2(gwl_data_stats, file = "gwl_data_stats.csv")
  ...
  readr::write_csv2(gwq_data_stats, file = "gwq_data_stats.csv")
