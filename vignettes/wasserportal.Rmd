...
swl_data <- stats::setNames(lapply(names(swl_data_list), function(name) {
  ...
  dplyr::mutate(Tagesmittelwert_Pegelstand_mNN = as.numeric(.data$Pegelnullpunkt_m) + .data$Tagesmittelwert_cm_ueber_Pegelnullpunkt/100) %>% 
...
gwl_master <-
  ...
  ### fix to remove 3 surface water stations from dataset 
  ### ()
  dplyr::filter(is.na(.data$Flusskilometer)) %>% 
...
readr::write_csv2(gwq_chloride_master_stats,
