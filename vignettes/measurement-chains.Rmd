...
con_is_defined <- sapply(con, stringr::str_length) > 0L

con_defined <- all(con_is_defined)


nc_vars <- sprintf("NEXTCLOUD_%s", c("URL", "USER", "PASSWORD"))

nc <- list(
  server = Sys.getenv(nc_vars[1]),
  user = Sys.getenv(nc_vars[2]),
  pw = Sys.getenv(nc_vars[3])
)

nc_is_defined <- sapply(nc, stringr::str_length) > 0L

nc_defined <- all(nc_is_defined)
...
### Export "mc_files" and "mc_metadata" to csv
...
metadata_path <- write_csv(mc_metadata, 
...
`r basename(data_csv_path)` file is 
`r round(size_data_csv / size_data_zip, 1)` 
times larger compared to compressing it into 
`r `basename(data_zip_path)`.
...
upload_files <- c(metadata_path, 
                  files_path,
                  data_zip_path)

res <- sapply(upload_files, function(upload_file) {
  kwb.utils::catAndRun(messageText = sprintf("Uploading file %s", upload_file),
                       expr = {
                       kwb.nextcloud::upload_file(file = upload_file,
...
```{r measurementchains_data-upload,  echo = FALSE, eval = con_defined & is_ghactions}
