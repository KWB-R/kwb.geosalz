...
all_sftp_paths <- kwb.utils::selectColumns(mc_files, "sftp_path")

# Download existing data from nextcloud
old_mc_data <- kwb.geosalz::get_measurement_chain_data_on_cloud()
  
# Get SFTP paths from which data were originally retrieved
old_sftp_paths <- kwb.utils::getAttribute(old_mc_data, "sftp_paths")
  
# Determine the paths to the new files that need to be downloaded
new_sftp_paths <- setdiff(all_sftp_paths, old_sftp_paths)

if (length(new_sftp_paths) > 0L) {
...
# Set the current data to the old data
mc_data <- old_mc_data

# If new files have been downloaded, import them into R
if (nrow(csv_files) > 0L) {
  
  # Import csv files using multiple CPU cores
  new_mc_data <- kwb.geosalz::read_measurementchains_data(
    csv_files,
    run_parallel = TRUE,
    debug = TRUE
  )
...
The following datasets were imported into or already available in R: 
