...
One measurement chain consist of six sensors (column `sensor_endnummer` in `Table 1`), 
which will be placed into one production well, but at different filter depths. 
In total three measurement chains will be installed into three different 
production wells (column: `brunnen_nummer` in `Table 1`) as shown in detail in 
`Table 1` below: 
...
Each sensor (type: `WLF05`, `link_to_manufacturer_factsheet`: `to_do: add-link-to-factsheet-on-kwb-cloud`) measures the the following two parameters with the characteristics summarized 
below:
...
assure that the `measurement chains` data (i.e. parameters `electrical conductivity` 
and `temperature`) will be downloaded from the `SFTP server` to a user-defined 
directory on your local device. For auto
...
The downloaded `.csv` measurement chains files will be imported into R by running 
the code below:


```{r measurementchains_data-import,  eval = con_defined}
stopifnot(nrow(csv_files)>0) ### print error in case no csv file is available
...
`r max(mc_data_parallel_stats$datetime_min)` with a total of `r sum(mc_data_parallel_stats$number_of_samples)` samples.
...
### Export to csv file
csv_path <- kwb.geosalz::write_measurementchains_data(mc_data_parallel,
                                                      target_directory,
                                                      to_zip = FALSE,
                                                      debug = debug)
...
zip_path <- kwb.geosalz::write_measurementchains_data(mc_data_parallel,
                                                      target_directory,
                                                      to_zip = TRUE,
                                                      debug = debug)
...
The file size of the `r basename(csv_path)` file is `r round(size_csv / size_zip, 1)` 
times larger compared to compressing it into `r `basename(zip_path)`.


### Data Upload 


In order to automate the data management process the [exported data](#data-export)
it is planned to upload these data to a restricted shared folder on the KWB 
cloud. 
